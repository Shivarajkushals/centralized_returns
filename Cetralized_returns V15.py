...
# Add this inside your application code after your current page options
    ...
    # # *** SIMPLIFIED: Just use sidebar navigation (removed toggle complexity) ***
    # selected_page = st.sidebar.radio("Select Page", options, key="upload_page")
    
    # with st.sidebar:
    #     st.markdown("---")
    #     st.info("💡 Use the menu above to navigate between pages")
    ...
    if selected_page == "RTV page":
        ...
        # *** FIXED SELECT ALL PATTERN ***
        st.markdown("### Select Stores")
        
        # Checkbox for Select All
        select_all = st.checkbox(
            "Select All Stores", 
            value=st.session_state.sr_select_all_state,
            key="sr_select_all_checkbox"
        )

        # Detect change and update
        if select_all != st.session_state.sr_select_all_state:
            st.session_state.sr_select_all_state = select_all
            if select_all:
                st.session_state.sr_stores_list = store_names.copy()
            else:
                st.session_state.sr_stores_list = []
            st.rerun()

        # Multiselect always uses session state
        selected_stores = st.multiselect(
            "Select Store(s)",
            options=store_names,
            default=st.session_state.sr_stores_list,
            key="sr_stores_multi"
        )

        # Update session state when user manually changes selection
        if selected_stores != st.session_state.sr_stores_list:
            st.session_state.sr_stores_list = selected_stores
            # Auto-update checkbox based on manual selection
            if len(selected_stores) == len(store_names) and len(store_names) > 0:
                st.session_state.sr_select_all_state = True
            else:
                st.session_state.sr_select_all_state = False
        ...
        if st.button("✅ Continue", key="sr_continue"):
            if not selected_stores:
                ...
                WHERE 
                    ...
                    # Fetch sales data and store in session state
                    _, df_sales_2 = fetch_sales_data(DB_CONFIG, start_date, end_date, selected_stores)
                    
                    # *** FIXED: Store data in session state ***
                    st.session_state.sr_page_data = {
                        'df_sales_2': df_sales_2,
        ...
        # *** FIXED: Display PDF section if data exists in session state ***
        if st.session_state.sr_page_data is not None:
            df_sales_2 = st.session_state.sr_page_data['df_sales_2']
            
            st.markdown("---")
            st.write("SR PDF output:")
            st.dataframe(df_sales_2)
            ...
            if st.button("🔄 New Search", key="sr_new_search"):
                st.session_state.sr_page_data = None
                st.session_state.sr_stores_list = []
                st.session_state.sr_select_all_state = False
                st.rerun()
        ...
        select_all = st.checkbox(
            "Select All Stores", 
            value=st.session_state.to_select_all_state,
            key="to_select_all_checkbox"
        )

        # Detect change and update
        if select_all != st.session_state.to_select_all_state:
            st.session_state.to_select_all_state = select_all
            if select_all:
                st.session_state.to_stores_list = store_names.copy()
            else:
                st.session_state.to_stores_list = []
            st.rerun()
        ...
        selected_stores = st.multiselect(
            "Select Store(s)",
            options=store_names,
            default=st.session_state.to_stores_list,
            key="to_stores_multi"
        )

        # Update session state when user manually changes selection
        if selected_stores != st.session_state.to_stores_list:
            st.session_state.to_stores_list = selected_stores
            # Auto-update checkbox based on manual selection
            if len(selected_stores) == len(store_names) and len(store_names) > 0:
                st.session_state.to_select_all_state = True
            else:
                st.session_state.to_select_all_state = False
        ...
        if st.button("✅ Continue", key="to_continue"):
            if not selected_stores:
                ...
                try:
                    ...
                    # *** FIXED: Store data in session state ***
                    st.session_state.to_page_data = {
                        'df_sales_1': df_sales_1,
                        'start_date': start_date,
                        'end_date': end_date,
                        'selected_stores': selected_stores
                    }
        ...
        # *** FIXED: Display PDF section if data exists in session state ***
        if st.session_state.to_page_data is not None:
            df_sales_1 = st.session_state.to_page_data['df_sales_1']
            ...
            # Add a button to clear the data and start fresh
            if st.button("🔄 New Search", key="to_new_search"):
                st.session_state.to_page_data = None
                st.session_state.to_stores_list = []
                st.session_state.to_select_all_state = False
                st.rerun()
