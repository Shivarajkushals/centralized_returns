...
def fetch_sales_data(DB_CONFIG):
    try:
        # Connect to the database
        conn = mysql.connector.connect(**DB_CONFIG)
        cursor = conn.cursor()
...
def generate_sales_return_pdfs(df_sales_2):
    ...
    grouped_data = df_sales_2.groupby('Design')
...
# Add this inside your application code after your current page options
    ...
    if page == "Upload page":
        ...
        _, df_sales_2 = fetch_sales_data(DB_CONFIG)
        ...
        data = fetch_all_data()  # Fetch all required data in one go
        df_sales_1, _ = fetch_sales_data(DB_CONFIG)
