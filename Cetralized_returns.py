...
def update_hidden_status(sr_numbers):
    ...
    try:
        ...
        update_to_query = f"UPDATE tbl_wh_transfer_out SET hidden = 1 WHERE sr_id IN ({id_placeholders})"
    ...
    if selected_page == "RTV page":
        ...
        with col2:
            ...
            if uploaded_file is not None:
                ...
                # -----------------------------------------
                # NEW VALIDATION: CHECK QTY FROM UPLOADED VS DATABASE
                # -----------------------------------------

                # Step 1: Calculate uploaded qty per bill_no + combination_id
                uploaded_qty_df = (
                    uploaded_df.groupby(["bill no", "combination_id"])
                            .size()
                            .reset_index(name="uploaded_qty")
                )

                # Step 2: Prepare tuples for SQL
                qty_filter_tuples = list(
                    uploaded_qty_df[["bill no", "combination_id"]]
                    .drop_duplicates()
                    .itertuples(index=False, name=None)
                )

                placeholders_qty = ", ".join(["(%s, %s)"] * len(qty_filter_tuples))
                flat_values_qty = [item for t in qty_filter_tuples for item in t]

                # Step 3: Query database qty
                conn = mysql.connector.connect(**DB_CONFIG)
                cursor = conn.cursor(dictionary=True)
                ...
                qty_query = f"""
                    ...
                    FROM tbl_sales t1
                    LEFT JOIN minimized_sales_register t2 ON t1.bill_date = t2.bill_date AND t1.bill_number = t2.bill_number
                    WHERE (t2.GST_bill_number, t1.combination_id) IN ({placeholders_qty})
                    AND t1.bill_date >= CURDATE() - INTERVAL 181 DAY
                    GROUP BY t1.bill_number, t1.combination_id;
                """

                cursor.execute(qty_query, flat_values_qty)
                db_qty_data = cursor.fetchall()
                cursor.close()
                conn.close()

                db_qty_df = pd.DataFrame(db_qty_data)

                # Step 4: Compare uploaded qty vs db qty
                qty_compare_df = uploaded_qty_df.merge(
                    db_qty_df,
                    on=["bill no", "combination_id"],
                    how="left"
                )
                ...
                # Step 5: Identify mismatched qty - uploaded MORE than sold
                ...
                if not qty_mismatch_df.empty:
                    st.error("❌ QTY MISMATCH: Cannot return more items than were sold!")
                    ...
                    display_mismatch = qty_mismatch_df.rename(columns={
                        ...
                        "db_qty": "Database Qty (Actual Sold)",
                        "excess_qty": "Excess Qty"
                    })
                    
                    st.dataframe(display_mismatch, use_container_width=True)
                    st.warning("⚠️ You are trying to return MORE items than were originally sold!")
                    st.stop()   # 🚫 STOP further RTV execution immediately
